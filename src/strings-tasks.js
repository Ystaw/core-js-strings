...
function extractNameFromTemplate(value) {
  const trimmedValue = value.replace('Hello, ', '').replace('!', '').trim();
  return trimmedValue;
}
...
function unbracketTag(str) {
  const trimmedStr = str.replace('<', '').replace('>', '').trim();
  return trimmedStr;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function Hash(c) {
